...
def main(argv=None):
    ...
    with tf.Graph().as_default(), tf.device("/cpu:0"):
        ...
        input_iterator = tf.compat.v1.data.Iterator.from_string_handle(handle, train_dataset.output_types, train_dataset.output_shapes)

        global_step = tf.Variable(0, trainable=False)
        learning_rate = tf.compat.v1.train.exponential_decay(float(params['lr']), global_step,
...
#                with tf.name_scope("GPU_%d" % i):
        ...
        for grad, var in grads:
            if grad is not None:
                tf.compat.v1.summary.histogram("gradients_on_average/%s" % var.op.name, grad)
        ...
        variable_to_average = (tf.compat.v1.trainable_variables() + tf.compat.v1.moving_average_variables())
        ...
        pred_result_image = tf.compat.v1.placeholder(tf.float32, shape=[params['batchsize'], 480, 640, 3])
        pred_result__summary = tf.compat.v1.summary.image("pred_result_image", pred_result_image, params['batchsize'])
